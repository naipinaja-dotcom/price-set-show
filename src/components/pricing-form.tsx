  ...
  switch (type) {
    ...
    case "threshold_multiple":
      return {
        ...
        default: {
          threshold: Number(f.threshold.default_threshold) || 0,
          rate: parseRupiah(f.threshold.default_rate),
        },
    ...
    case "attendance":
      return {
        ...
        overtime: f.attendance.overtimeOn
          ? { enabled: true, rate_per_hour: parseRupiah(f.attendance.overtime_rate_per_hour) }
          : null,
        incentives: f.attendance.incentives
          .filter((c) => c.label.trim())
          .map((c) => ({
            label: c.label.trim(),
            amount: parseRupiah(c.amount),
...
function buildEnvelope(type: PricingCalcType, schemeFor: SchemeFor, f: FormState): PricingEnvelope {
  return {
    ...
    add_kg:
      (type === "flat_unit" || type === "threshold_multiple") && f.addKgOn
...
function stepTierBreakdown(
  s: StepTierState,
  value: number,
  unit: string,
): { steps: ExStep[]; total: number } {
  ...
  for (const t of s.tiers) {
    ...
    if (value > lo) {
      ...
      steps.push({
        text: `${lo}–${hiLabel} ${unit}: ${count} step × ${formatRupiah(perStep)}`,
        amount: add,
      });
    }
  }
...
function buildExample(
  ...
  const addModifierNotes = () => {
    if ((calcType === "flat_unit" || calcType === "threshold_multiple") && f.addKgOn)
      notes.push(
        "Add-KG nyala: biaya berat ditambah DI ATAS hasil ini, sesuai berat tiap kiriman.",
      );
    if (f.multiDropOn)
      notes.push(
        `Multi-drop nyala: kiriman ke-2 dst di hari yang sama +${formatRupiah(parseRupiah(f.multiDropFee))} per kiriman.`,
      );
    if (schemeFor === "client" && f.billingOn)
      notes.push(
        "Billing client nyala: total tagihan masih ditambah min charge / admin fee & PPN di akhir.",
      );
  };
  ...
  switch (calcType) {
    case "flat_unit": {
      if (f.flatUnit.rate_by === "flat") {
        ...
        addModifierNotes();
        return {
          scenario: `Contoh: 3 ${unitWord}.`,
          steps,
          total: { label: `Total 3 ${unitWord}`, amount: rate * 3 },
          notes,
        };
      }
      ...
      addModifierNotes();
      return {
        scenario: `Contoh: 1 kiriman, tarif ikut kolom "${col}".`,
        steps,
        total: { label: `Fee 1 kiriman ke "${first?.key ?? "-"}"`, amount: rate },
        notes,
      };
    }
    case "tier":
    case "tier_daily": {
      ...
      if (f.tier.distanceOn && f.tier.weightOn)
        notes.push(
          "Jarak & berat dua-duanya nyala → hasil berat dihitung dengan cara yang sama, lalu DIJUMLAH ke hasil ini.",
        );
      addModifierNotes();
    ...
    case "threshold_multiple": {
      ...
      addModifierNotes();
      return {
        scenario: `Contoh: store "${label}" total ${sampleKg} kg dalam sehari.`,
        steps,
        total: { label: "Fee store hari itu", amount: fee },
        notes,
      };
    }
    case "attendance": {
      ...
      const res = calcAttendanceScheme(env, [
        {
          log_date: "2026-01-01",
          duration_minutes: Math.round(worked * 60),
          is_late: false,
          is_absent: false,
        },
      ]);
      ...
      f.attendance.incentives
        ...
        .forEach((c) => {
          ...
          steps.push({
            text: `+ ${c.label} ${c.condition === "always" ? "(selalu)" : "(status ONTIME ✓)"}`,
            amount: amt,
          });
        });
      notes.push(
        'Kalau statusnya LATE, insentif bersyarat "ONTIME" jadi Rp0 (biner, tidak setengah).',
      );
      return {
        scenario: `Contoh: 1 hari kerja ${worked} jam, status ONTIME.`,
        steps,
        total: { label: "Fee hari itu", amount: row?.fee ?? 0 },
        notes,
      };
    }
  }
...
function loadForm(scheme: PricingScheme | undefined): {
  ...
  if (!scheme || !scheme.params || scheme.params.version !== 1) {
    return {
      form,
      calcType: validType(scheme?.calc_type) ? (scheme!.calc_type as PricingCalcType) : "flat_unit",
      schemeFor: scheme?.scheme_for ?? "rider",
    };
  }
  ...
  if (type === "flat_unit") {
    ...
    form.attendance = {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      incentives: (c.incentives ?? []).map((x: any) => ({
        label: x.label ?? "",
        amount: String(x.amount ?? ""),
        condition: x.condition === "ontime_only" ? "ontime_only" : "always",
      })),
    };
  }
...
function StepTierEditor({
  value,
  onChange,
  unit,
}: {
  value: StepTierState;
  onChange: (v: StepTierState) => void;
  unit: "km" | "kg";
}) {
  const setTier = (i: number, patch: Partial<StepTierState["tiers"][number]>) =>
    onChange({
      ...value,
      tiers: value.tiers.map((t, idx) => (idx === i ? { ...t, ...patch } : t)),
    });
  const addTier = () =>
    onChange({
      ...value,
      tiers: [
        ...value.tiers,
        { from: value.base_until || "0", to: "", step: "1", add_per_step: "" },
      ],
    });
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="flex flex-col gap-1.5">
          ...
          <RupiahInput
            ...
            onChange={(v) => onChange({ ...value, base_fee: v })}
          />
        </div>
        <div className="flex flex-col gap-1.5">
          <FieldLabel>Base sampai ({unit})</FieldLabel>
          <TextInput
            value={value.base_until}
            inputMode="decimal"
            onChange={(e) => onChange({ ...value, base_until: e.target.value })}
          />
        </div>
      </div>
      <TableShell>
        ...
        {value.tiers.map((t, i) => (
          <tr key={i} className="border-t border-border/60">
            ...
            <Td>
              <RupiahInput
                value={t.add_per_step}
                onChange={(v) => setTier(i, { add_per_step: v })}
              />
            </Td>
            <Td className="text-center">
              <RowDeleteBtn onClick={() => delTier(i)} />
            </Td>
          </tr>
        ))}
      </TableShell>
...
function PricingFormInner({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      posthog.capture(mode === "create" ? "pricing_scheme_created" : "pricing_scheme_updated", {
        calc_type: calcType,
        scheme_for: schemeFor,
      });
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    patch({
                      flatUnit: { ...f.flatUnit, unit: e.target.value as FlatUnitState["unit"] },
                    })
                  }
                ...
                <select
                  ...
                  onChange={(e) =>
                    patch({
                      flatUnit: {
                        ...f.flatUnit,
                        rate_by: e.target.value as FlatUnitState["rate_by"],
                      },
                    })
                  }
                  className="w-full text-sm rounded-md border border-border bg-card px-2.5 py-1.5"
            ...
            {f.flatUnit.rate_by === "flat" ? (
              <div className="flex flex-col gap-1.5 max-w-xs">
                ...
                <RupiahInput
                  value={f.flatUnit.flat_rate}
                  onChange={(v) => patch({ flatUnit: { ...f.flatUnit, flat_rate: v } })}
                />
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      value={
                        /service|layanan/i.test(f.flatUnit.match_column)
                          ? "Service Type"
                          : /delivery type|return|tipe kirim/i.test(f.flatUnit.match_column)
                            ? "Delivery Type"
                            : "Area"
                      }
                      onChange={(e) =>
                        patch({ flatUnit: { ...f.flatUnit, match_column: e.target.value } })
                      }
                      className="w-full text-sm rounded-md border border-border bg-card px-2.5 py-1.5 outline-none focus:ring-2 focus:ring-ring"
                    >
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      "Antar/Kembali" dideteksi otomatis oleh sistem — ga perlu ada kolomnya di file
                      CSV.
                    </p>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <RupiahInput
                      value={f.flatUnit.default_rate}
                      onChange={(v) => patch({ flatUnit: { ...f.flatUnit, default_rate: v } })}
                    />
                  </div>
                </div>
                <TableShell>
                  <>
                    <Th>
                      {/delivery type|return|tipe kirim/i.test(f.flatUnit.match_column)
                        ? "Nilai (DELIVERY / RETURN)"
                        : /service|layanan/i.test(f.flatUnit.match_column)
                          ? "Nilai (cth: INSTANT)"
                          : "Nilai Kolom (cth: Jakarta Pusat)"}
                    </Th>
                        ...
                        <TextInput
                          ...
                          onChange={(e) =>
                            patch({
                              flatUnit: {
                                ...f.flatUnit,
                                rates: f.flatUnit.rates.map((x, idx) =>
                        ...
                        <RowDeleteBtn
                          onClick={() =>
                            patch({
                              flatUnit: {
                                ...f.flatUnit,
                                rates: f.flatUnit.rates.filter((_, idx) => idx !== i),
                              },
                            })
                          }
                        />
                ...
                <AddRowBtn
                  onClick={() =>
                    patch({
                      flatUnit: {
                        ...f.flatUnit,
                        rates: [...f.flatUnit.rates, { key: "", rate: "" }],
                      },
                    })
                  }
                >
          ...
          <div className="space-y-3">
            {calcType === "tier_daily" && (
              <div className="rounded-md border border-warning/30 bg-warning/10 px-3.5 py-2.5 text-xs text-warning">
                Akumulasi harian: jarak/berat semua kiriman 1 rider dalam 1 hari dijumlah dulu, baru
                dihitung.
              </div>
            )}
            ...
            {tierSubtab === "distance" && (
              <ToggleBlock
                label="Pakai Jarak (km)"
                on={f.tier.distanceOn}
                onToggle={(on) => patch({ tier: { ...f.tier, distanceOn: on } })}
              >
                <StepTierEditor
                  unit="km"
                  value={f.tier.distance}
                  onChange={(v) => patch({ tier: { ...f.tier, distance: v } })}
                />
              </ToggleBlock>
            )}
            {tierSubtab === "weight" && (
              <ToggleBlock
                label="Pakai Berat (kg)"
                on={f.tier.weightOn}
                onToggle={(on) => patch({ tier: { ...f.tier, weightOn: on } })}
              >
                <StepTierEditor
                  unit="kg"
                  value={f.tier.weight}
                  onChange={(v) => patch({ tier: { ...f.tier, weight: v } })}
                />
              </ToggleBlock>
            )}
            {f.tier.distanceOn && f.tier.weightOn && (
              <p className="text-[11px] text-muted-foreground">
                Jarak & berat dua-duanya aktif → hasilnya dijumlah.
              </p>
            )}
          </div>
        )}
        ...
        {calcType === "threshold_multiple" && (
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">
              Qty dibaca dari berat aktual (kg). Fee per grup = ceil(total kg / threshold) × rate.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="flex flex-col gap-1.5">
                <FieldLabel>Kolom pengelompokan</FieldLabel>
                <TextInput
                  value={f.threshold.group_by}
                  onChange={(e) =>
                    patch({ threshold: { ...f.threshold, group_by: e.target.value } })
                  }
                  placeholder="Area"
                />
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <TextInput
                  value={f.threshold.default_threshold}
                  inputMode="decimal"
                  onChange={(e) =>
                    patch({ threshold: { ...f.threshold, default_threshold: e.target.value } })
                  }
                />
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <RupiahInput
                  value={f.threshold.default_rate}
                  onChange={(v) => patch({ threshold: { ...f.threshold, default_rate: v } })}
                />
              </div>
            </div>
            <TableShell>
                    ...
                    <TextInput
                      ...
                      onChange={(e) =>
                        patch({
                          threshold: {
                            ...f.threshold,
                            rules: f.threshold.rules.map((x, idx) =>
                              idx === i ? { ...x, key: e.target.value } : x,
                            ),
                          },
                        })
                      ...
                      onChange={(e) =>
                        patch({
                          threshold: {
                            ...f.threshold,
                            rules: f.threshold.rules.map((x, idx) =>
                              idx === i ? { ...x, threshold: e.target.value } : x,
                            ),
                          },
                        })
                      }
                    ...
                    <RowDeleteBtn
                      onClick={() =>
                        patch({
                          threshold: {
                            ...f.threshold,
                            rules: f.threshold.rules.filter((_, idx) => idx !== i),
                          },
                        })
                      }
                    />
            ...
            <AddRowBtn
              onClick={() =>
                patch({
                  threshold: {
                    ...f.threshold,
                    rules: [...f.threshold.rules, { key: "", threshold: "", rate: "" }],
                  },
                })
              }
            >
        ...
        {calcType === "attendance" && (
          <div className="space-y-4">
            <div className="rounded-md border border-primary-border bg-primary-soft px-3.5 py-2.5 text-xs text-primary-soft-foreground">
              Rumus: (fee penuh × proporsi jam kerja) {f.attendance.overtimeOn ? "+ lembur " : ""}+
              insentif (nominal ditentuin di sini, data absensi cuma dipakai cek syarat — mis.
              OTP=ONTIME).
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <RupiahInput
                  value={f.attendance.full_fee}
                  onChange={(v) => patch({ attendance: { ...f.attendance, full_fee: v } })}
                />
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <TextInput
                  type="number"
                  value={f.attendance.standard_hours}
                  onChange={(e) =>
                    patch({ attendance: { ...f.attendance, standard_hours: e.target.value } })
                  }
                  placeholder="8"
                />
              </div>
            ...
            <p className="text-[11px] text-muted-foreground -mt-2">
              Kerja kurang dari jam standar dibayar proporsional. Kerja pas/lebih = fee penuh
              (kecuali lembur dinyalain di bawah).
            </p>
            ...
            >
              <div className="max-w-xs">
                ...
                <RupiahInput
                  value={f.attendance.overtime_rate_per_hour}
                  onChange={(v) =>
                    patch({ attendance: { ...f.attendance, overtime_rate_per_hour: v } })
                  }
                />
              </div>
            </ToggleBlock>
            ...
            <div>
                ...
                {f.attendance.incentives.map((c, i) => (
                  <tr key={i} className="border-t border-border/60">
                    <Td>
                      <TextInput
                        value={c.label}
                        placeholder="cth: Insentif Ontime"
                        onChange={(e) =>
                          patch({
                            attendance: {
                              ...f.attendance,
                              incentives: f.attendance.incentives.map((x, idx) =>
                        ...
                        onChange={(e) =>
                          patch({
                            attendance: {
                              ...
                              incentives: f.attendance.incentives.map((x, idx) =>
                                idx === i
                                  ? { ...x, condition: e.target.value as "always" | "ontime_only" }
                                  : x,
                              ),
                            },
                          })
                      ...
                      <RowDeleteBtn
                        onClick={() =>
                          patch({
                            attendance: {
                              ...f.attendance,
                              incentives: f.attendance.incentives.filter((_, idx) => idx !== i),
                            },
                          })
                        }
                      />
              ...
              <p className="text-[11px] text-muted-foreground mt-1.5">
                "Cuma kalau ONTIME" itu biner — hari LATE dapet Rp0 buat insentif ini, ga ada
                setengah-setengah.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-primary-border/60 bg-primary-soft/40 px-4 py-3.5 mt-4">
            ...
            <div className="flex items-center justify-between gap-3 mt-2.5 pt-2.5 border-t border-primary-border/50">
              ...
              <span className="text-base font-bold text-primary tabular-nums">
                {formatRupiah(example.total.amount)}
              </span>
            </div>
