...
import {
  LayoutDashboard, Users, UserCircle2, Tag, Upload, Wallet,
  FileBarChart2, Calculator, Coins, TrendingUp, LayoutPanelTop,
  ShieldCheck, Search, Receipt, LogOut, Menu, X, Package,
  Banknote, Percent, Bike, Bell, BellRing, ChevronLeft,
...
const NAV_PAYROLL = [
  { to: "/admin/dashboard",  label: "Payroll Dashboard",  icon: LayoutDashboard, section: "OPERATIONS" },
...
const NAV_INTELLIGENCE = [
  { to: "/admin/pnl-dashboard",      label: "Executive Dashboard", icon: LayoutPanelTop, section: "OVERVIEW" },
  { to: "/admin/pnl",               label: "Margin Analytics",    icon: TrendingUp,     section: "ANALYTICS" },
  { to: "/admin/revenue-analytics", label: "Revenue Analytics",   icon: Banknote,       section: "ANALYTICS" },
  { to: "/admin/bcr-analytics",     label: "BCR Analytics",       icon: Percent,        section: "ANALYTICS" },
  { to: "/admin/shipment-analytics",label: "Shipment Analytics",  icon: Package,        section: "ANALYTICS" },
...
export function AdminLayout({ children, title, subtitle }: { children: ReactNode; title: string; subtitle?: string }) {
  ...
  useEffect(() => { setMode(modeForPath(pathname)); }, [pathname]);

  useEffect(() => { localStorage.setItem(NAV_MODE_KEY, mode); }, [mode]);
  ...
  const switchMode = useCallback((m: NavMode) => {
    if (m === mode) return;
    setMode(m);
  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    <>
      {/* Brand */}
      <div className={`flex items-center gap-3 border-b border-border ${collapsed && !mobile ? "px-4 py-[18px] justify-center" : "px-5 py-[18px]"}`}>
      ...
      <nav className="flex-1 px-2.5 py-3 overflow-y-auto space-y-3">
        {navGroups.map(({ section, items }) => (
          <div key={section}>
            {(!collapsed || mobile) && (
              <div className="px-3 pb-1 pt-0.5 text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">{section}</div>
            )}
              ...
              {items.map((it) => {
                ...
                return (
                  <Link key={it.to} to={it.to}
                    ...
                    className={"flex items-center rounded-md text-[13px] transition-all duration-150 " +
                      (collapsed && !mobile ? "justify-center px-0 py-2.5" : "gap-2.5 px-3 py-[7px]") + " " +
                      (active ? "bg-primary-soft text-primary-soft-foreground font-semibold" : "text-foreground/65 hover:bg-muted/80 hover:text-foreground")}>
                    <Icon className={`flex-shrink-0 ${collapsed && !mobile ? "w-[18px] h-[18px]" : "w-4 h-4"} ${active ? "text-primary" : ""}`} />
      ...
      <div className="border-t border-border p-3">
        {(!collapsed || mobile) ? (
          <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-muted/60 transition-colors">
            ...
            <button onClick={handleLogout} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground hover:text-destructive transition-colors" title="Logout" aria-label="Logout">
              <LogOut className="w-3.5 h-3.5" />
            </button>
          </div>
        ) : (
          <button onClick={handleLogout} className="w-full flex justify-center p-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-muted transition-colors" title="Logout" aria-label="Logout">
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      ...
      {/* Desktop sidebar */}
      <aside className={`hidden lg:flex flex-col border-r border-border bg-sidebar shadow-[1px_0_0_0_var(--color-border)] flex-shrink-0 transition-[width] duration-250 ease-[cubic-bezier(0.4,0,0.2,1)] ${collapsed ? "w-[72px]" : "w-60"}`}>
        <SidebarContent />
      </aside>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-64 bg-sidebar border-r border-border flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border">
              ...
              <button onClick={() => setMobileOpen(false)} className="p-1" aria-label="Tutup menu"><X className="w-5 h-5" /></button>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-10 h-14 border-b border-border bg-card/85 backdrop-blur-md flex items-center px-4 lg:px-6 gap-3">
          ...
          {/* Mobile hamburger */}
          <button className="lg:hidden p-1.5 -ml-1 rounded-md hover:bg-muted" onClick={() => setMobileOpen(true)} aria-label="Buka menu">
