...
function PayrollPage() {
  ...
  const generate = async () => {
    ...
    for (const rider of riders ?? []) {
      ...
      const attendanceFee = (rAttendDetails ?? []).reduce(
        (s: number, a: any) => s + Number(a.fee || 0),
        0,
      );
  ...
  const finalize = async () => {
    ...
    posthog.capture("payroll_finalized", {
      ...
      period_type: activeRun.period_type,
    });
    ...
    <AdminLayout title="Payroll">
      <div className="flex gap-6">
        ...
        <section className="flex-1 min-w-0">
          {!activeRun ? (
            <p className="text-sm text-muted-foreground">Pilih atau buat payroll run.</p>
          ) : (
            <>
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {activeRun.period_start} → {activeRun.period_end} · status:{" "}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={finalize}
                    disabled={activeRun.status !== "draft" || finalizing}
                    className="rounded-md bg-warning text-warning-foreground px-3 py-1.5 text-sm disabled:opacity-50 inline-flex items-center gap-1"
                  >
                    {finalizing ? (
                  ...
                  <button
                    onClick={publish}
                    disabled={activeRun.status === "published" || publishing}
                    className="rounded-md bg-primary text-primary-foreground px-3 py-1.5 text-sm disabled:opacity-50 inline-flex items-center gap-1"
                  >
                    {publishing ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : (
                    ...
                    {details.length === 0 ? (
                      <tr>
                        <td colSpan={9} className="p-6 text-center text-muted-foreground">
                          Belum ada detail — klik Generate Detail
                        </td>
                      </tr>
                    ) : (
                      pagedDetails.map((d) => (
                        <tr key={d.id} className="border-t border-border">
                          <td className="p-2">
                            <div className="font-medium">{d.riders?.full_name}</div>
                            <div className="text-xs text-muted-foreground">
