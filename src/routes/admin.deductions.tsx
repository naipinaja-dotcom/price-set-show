          ...
          {nf.auto_recurring && (
            <div className="mt-3">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Angka ini otomatis dipotong ke tiap rider tiap kali payroll digenerate.
              </p>
            </div>
          )}
...
function AddTab() {
  ...
  const save = async () => {
    ...
    if (!f.deduction_type_id || !f.total_amount)
      return toast.error("Lengkapi jenis & nominal potongan");
    setSaving(true);
...
function ActiveTab() {
  ...
  const saveEdit = async (r: Inst) => {
    if (!ef.deduction_type_id || !ef.total_amount)
      return toast.error("Lengkapi jenis & nominal potongan");
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        installment_count: ef.installment_count,
        per_period_amount: per,
              ...
              paged.map((r) => (
                <Fragment key={r.id}>
                  <tr className="border-b border-border last:border-b-0 hover:bg-muted/40 transition-colors">
                    ...
                    <td className="p-3 text-muted-foreground">
                      Rp{Number(r.per_period_amount).toLocaleString("id-ID")}
                    </td>
                    <td className="p-3 text-muted-foreground">
                      {r.installments_paid}/{r.installment_count}
                    </td>
                    <td className="p-3 text-muted-foreground">{r.next_deduction_date ?? "—"}</td>
