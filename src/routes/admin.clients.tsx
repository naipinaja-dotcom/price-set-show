...
type Client = {
  ...
  address: string | null; contact_person: string | null; phone: string | null;
...
function ClientsPage() {
  ...
  const load = async () => {
    setLoading(true);
    const [{ data: clientData, error }, { data: schemes }] = await Promise.all([
      supabase.from("clients").select("id, code, name, address, contact_person, phone, active, created_at").order("name"),
      (supabase as any).from("pricing_schemes").select("client_id, calc_type").eq("scheme_for", "client"),
    ]);
    if (error) toast.error(error.message); else setRows(clientData ?? []);
  ...
  return (
    <AdminLayout title="Client" subtitle={`${rows.length} client terdaftar`}>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <button onClick={() => downloadCSV(filtered, schemeOf)}
      ...
      <div className="rounded-xl border border-border overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="p-6 text-center text-muted-foreground"><Loader2 className="w-4 h-4 inline animate-spin mr-1" />Memuat…</td></tr>
            ) : filtered.length === 0 ? (
              ...
              return (
                <tr key={c.id} className={`border-t border-border hover:bg-muted/30 transition-colors ${i % 2 === 1 ? "bg-muted/10" : "bg-card"}`}>
                  <td className="px-3 py-2.5 text-muted-foreground tabular-nums text-[12px]">{i + 1}</td>
                  <td className="px-3 py-2.5 font-mono text-[12px] font-semibold">{c.code}</td>
                  <td className="px-3 py-2.5 font-medium">{c.name}</td>
                  <td className="px-3 py-2.5">
                    {scheme
                      ? <span className="text-[11px] font-medium bg-primary/10 text-primary px-2 py-0.5 rounded-full">{scheme}</span>
                      : <span className="text-[11px] text-muted-foreground">—</span>}
                  </td>
                  <td className="px-3 py-2.5 text-[12px] text-muted-foreground">
                    {c.created_at ? new Date(c.created_at).toLocaleDateString("id-ID", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "—"}
                  </td>
                  <td className="px-3 py-2.5 text-right">
                    <button onClick={() => { setEdit(c); setOpen(true); }} className="p-1.5 hover:bg-muted rounded-md transition-colors" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
                    <button onClick={() => remove(c.id)} className="p-1.5 hover:bg-muted rounded-md text-destructive transition-colors" title="Hapus"><Trash2 className="w-3.5 h-3.5" /></button>
                  </td>
                </tr>
              );
            })}
      ...
      <div className="bg-card rounded-xl w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3 text-sm">
          {(["code","name","address","contact_person","phone"] as const).map((f) => (
            <div key={f}>
              <label className="text-xs text-muted-foreground font-medium capitalize">{f.replace(/_/g, " ")}</label>
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button onClick={onClose} className="px-3 py-1.5 text-sm rounded-lg border border-border hover:bg-muted transition-colors">Batal</button>
          <button onClick={save} disabled={saving} className="px-3 py-1.5 text-sm rounded-lg bg-primary text-primary-foreground disabled:opacity-50 hover:opacity-90 transition-opacity">
