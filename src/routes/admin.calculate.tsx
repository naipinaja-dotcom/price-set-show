...
import { calcScheme, type DeliveryRow, type CalcResult } from "@/lib/pricing-calc";
...
function CalculatePage() {
  ...
  const commit = async () => {
    if (!result || !ranScheme || ranScheme.scheme_for !== "rider") return;
    const rows = result.perRow.filter((r) => r.id);
    if (rows.length === 0) return toast.error("Tidak ada baris untuk disimpan.");
    if (!confirm(`Simpan fee ke ${rows.length} baris pengiriman? Angka ini yang akan dipakai Payroll Run.`)) return;
    setCommitting(true);
    try {
      const chunkSize = 100;
      let done = 0;
      for (let i = 0; i < rows.length; i += chunkSize) {
        ...
        const res = await Promise.all(chunk.map((r) => supabase.from("delivery_records").update({ fee: r.fee }).eq("id", r.id as string)));
