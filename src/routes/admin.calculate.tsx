  ...
  const run = async () => {
    ...
    try {
      if (scheme.calc_type === "attendance") {
        ...
        const riderIds = [
          ...new Set(rowsPlain.map((r) => r.rider_id).filter((id): id is string => Boolean(id))),
        ];
        ...
        if (riderIds.length > 0) {
          ...
          riderMap = Object.fromEntries(
            (riderData ?? []).map((r) => [
              r.id,
              { full_name: r.full_name, employee_id: r.employee_id },
            ]),
          );
        }
        ...
        posthog.capture("fee_calculation_run", {
          ...
          period_from: from,
          period_to: to,
          completed_rows: res.completedRows,
          skipped_rows: res.skippedRows,
        });
        setResult(res);
        setRanScheme(scheme);
      }
  ...
  const commit = async () => {
    ...
    try {
      ...
      posthog.capture("fee_committed_to_payroll", {
        calc_type: ranScheme.calc_type,
        scheme_for: ranScheme.scheme_for,
        committed_rows: done,
      });
