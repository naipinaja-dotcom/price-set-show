...
type RiderStatus = "active" | "inactive" | "pending_review" | "suspended" | "ready_to_work" | "ready" | "available" | "hire" | "probation" | "contract";
type Rider = {
...
function RiderImportModal({ clients, onClose, onDone }:
  ...
  const FIELD_ALIASES: Record<string, string[]> = {
    employee_id: ["employeeid", "employee", "id", "mtr", "kodemitra", "idkaryawan", "kodekaryawan"],
    nik: ["nik", "nomorinduk", "noktp"],
    full_name: ["fullname", "name", "nama", "namalengkap"],
    phone: ["phone", "telepon", "telp", "hp", "nohp", "notelp", "nowa", "whatsapp", "wa"],
    email: ["email"],
    client: ["client", "klien"],
    status: ["status"],
    ...
    birth_date: ["birthdate", "birthdate", "tanggallahir", "ttl", "lahir"],
  ...
  const doImport = async () => {
    ...
    const validStatus = ["active", "hire", "ready_to_work", "ready", "available", "probation", "contract", "inactive", "pending_review", "suspended"];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const payload: any[] = [];
    rows.forEach((o, idx) => {
      const line = idx + 2;
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <select value={f.status} onChange={(e) => setF({ ...f, status: e.target.value as RiderStatus })}
              ...
              {["active","hire","ready_to_work","ready","available","probation","contract","pending_review","inactive","suspended"].map((s) => <option key={s} value={s}>{s.replace("_"," ")}</option>)}
            </select>
          </div>
