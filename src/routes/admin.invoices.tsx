...
function InvoicesPage() {
  ...
  const exportCSV = () => {
    const header = ["Client", "Periode Dari", "Periode Sampai", "Tipe Skema", "Subtotal", "Tambahan", "Total", "Status", "Tanggal Invoice"];
    const data = filtered.map((i) => [
      clientName(i.client_id), i.period_start ?? "", i.period_end ?? "", i.scheme_name ?? i.calculation_type ?? "",
    ...
    <AdminLayout title="Invoices" subtitle="Invoice client yang sudah di-commit dari Hitung Fee (sisi revenue)">
      ...
      <div className="rounded-lg border border-border overflow-x-auto">
        <table className="w-full text-sm whitespace-nowrap">
          ...
          <tbody>
            {loading ? <tr><td colSpan={8} className="p-6 text-center"><Loader2 className="w-4 h-4 animate-spin inline" /></td></tr>
            : filtered.length === 0 ? <tr><td colSpan={8} className="p-6 text-center text-muted-foreground">Belum ada invoice — commit dari halaman Hitung Fee dengan skema Client.</td></tr>
            : paged.map((i) => (
