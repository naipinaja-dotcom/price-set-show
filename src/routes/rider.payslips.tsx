...
type PayslipRow = {
  id: string; detail_id: string; published_at: string;
...
function PayslipsPage() {
  ...
  useEffect(() => {
    ...
    sb.from("payslips")
      ...
      .then(({ data }: { data: PayslipRow[] | null }) => { setSlips(data ?? []); setLoading(false); });
  }, [rider]);
    ...
    <RiderLayout title="Slip Gaji">
      {busy ? (
        ...
        <div className="rounded-lg border border-dashed border-border bg-card p-8 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Slip gaji akan muncul di sini setelah admin publish payroll.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {slips.map((s) => (
            <button key={s.id} onClick={() => setOpenSlip(s)}
      ...
      {openSlip && <PayslipDetailModal slip={openSlip} riderName={rider?.full_name ?? ""} employeeId={rider?.employee_id ?? ""} onClose={() => setOpenSlip(null)} />}
    </RiderLayout>
  );
}

function PayslipDetailModal({ slip, riderName, employeeId, onClose }:
  { slip: PayslipRow; riderName: string; employeeId: string; onClose: () => void }) {
  const [ded, setDed] = useState<{ name: string; amount: number }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    sb.from("payroll_deductions").select("amount, deduction_types(name)").eq("detail_id", slip.detail_id)
      .then(({ data }: { data: { amount: number; deduction_types: { name: string } | null }[] | null }) => {
        setDed((data ?? []).map((d) => ({ name: d.deduction_types?.name ?? "Potongan", amount: Number(d.amount) })));
        setLoading(false);
      });
  }, [slip.detail_id]);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 grid place-items-end sm:place-items-center z-50" onClick={onClose}>
      <div className="bg-card rounded-t-2xl sm:rounded-lg w-full sm:max-w-sm max-h-[85vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-border sticky top-0 bg-card">
          <div>
            <div className="text-sm font-semibold">{slip.payroll_runs?.name ?? "Payroll"}</div>
            <div className="text-[11px] text-muted-foreground">{riderName} · {employeeId}</div>
          </div>
        ...
        <div className="p-4 text-sm">
          ...
          {loading ? (
            ...
            ded.map((d, i) => <Row key={i} label={d.name} value={`−${formatRupiah(d.amount)}`} muted />)
          )}
          <div className="border-t border-border my-2" />
          <div className="flex items-baseline justify-between pt-1">
            <span className="text-xs text-muted-foreground">Take-home</span>
            <span className="text-xl font-semibold text-primary">{formatRupiah(slip.data?.net_pay)}</span>
          </div>
        </div>
      </div>
    </div>
